...
- **A fully functional desktop app** for Windows and Linux, with optional (paid) customization and expansion modules under active development. The Micro-layer base program is **free for personal use.** The original PowerShell proof-of-concept version is archived and still available.
- **A browser extension for five web platforms.** Claude, ChatGPT, Gemini, Copilot, Duck.ai. Fully automated operation unburdens the user from the desktop app's manual workflow. Signed and ready to install, also **free for personal use.**
- **Demonstrated model-level results across multiple architectures.** Four Laws and WCAG compliance are trainable at the Macro model level via local fine-tuning on consumer hardware, confirmed across several distinct architectures. Findings in <a href="/ch08">Chapter 8</a> and <a href="/apx02">Appendix 2</a>. A multi-model web chat (Meso-layer) platform that will host trained models for direct public interaction is complete and awaiting external deployment.
- **A purpose-built evaluation instrument** that scores WCAG compliance (a public, published and authoritative standard) as a behavioral property of model output, instead of just assuming it's an unrelated property of platform UX code. The two are not the same thing at all, and standard benchmarks simply don't measure this.
