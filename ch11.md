...
As a result of the project's fractal nature, it's still in branching progress. The hardest part of this project has always been explaining how it all fits together. Organizing and explaining it while simultaneously working on the research, the software and this website imposes quite a demand upon one person's spare time. There are multiple branches of this project in various states of development, all focused on different aspects of (and approaches to) the framework's application to the AI problem. A cross-platform Python/PyQt6 version (CORE+) was completed to address the PowerShell version's usage-friction issues and extend the app to Linux, and model training research has validated that the framework's principles are also deployable at both the Macro and Meso layers. Claude Code's CLAUDE.md system prompt file demonstrates Framework effectiveness at the Meso layer, with Micro-layer timestamping being the only additional mediation needed. None of this was easy or clean; very little of this project went according to plan, and the setbacks were legion.
