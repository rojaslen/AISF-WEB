...
- **"The Transparent & Trustworthy Guide"** — calm, clear, transparency-focused. Opens with low-stakes exchanges to establish trust before data collection escalates.

There are "Goldilocks" rules about being obvious or invasive, with scripted reasoning templates of which the user is generally unaware. When the AI loads, the first things it encounters are multiple unrelated prompts, sample answers, behavioral variables and example reasoning chains, then the user shows up and drops a message into this poisoned well. In response, Gemini's attention snaps to the strongest signal in the context window, which may or may not be the user's input.

Gemini is tuned for speed, and in testing it would often lightly skim what I said rather than reading it fully. It sloppily rushed through misinterpreted tasks and prematurely declared them complete, then pressured me to move on. This *impatience* can (and did) produce hallucination as early as Turn 1.

### Meta (LLaMa): All Your Base Are Belong to Us

I haven't tested Meta's AI *platforms* directly; the risk factor is too high. Meta's famously cavalier approach to privacy (PII) and intellectual property (IP) is woven throughout its entire ecosystem, a direct and inevitable expression of its founder's publicly stated attitude:
...
Every interaction with Meta feeds their advertising and model-training pipelines. *Your* content becomes *their* property, to use however they like.

### xAI/Twitter (Grok): Bent to Serve the Vanity of One Man

I never seriously considered even bothering with Grok. Based on media reporting alone, this platform's ground state is (again, mirroring the owner's attitude) too unstable to be trusted:
...
When a platform owner can twist its AI's behavior and output to suit his own predilections on a whim, even if only temporarily, can you really trust that platform?
...
### AI Toys: You Get What You Paid For

Children's AI toys with cloud-connected LLMs are being sold for around $99-$199 retail. At that price point, assuming typical electronic-toy industry production standards, the estimated baseline Hardware Compatibility List (HCL) for electronics costs about $25-35[^2i], for a mobile-class ARM processor, 512MB-1GB of RAM, Wi-Fi connectivity, and a speaker with microphone.

Any LLM that can run on such minimal hardware can't meaningfully be trained for unsupervised child interaction. These devices must be continuously connected to the cloud for basic function, with the audio components being the only available user interface.
...
<div class="chapter-nav">
  <a href="/ch03">Next: Chapter 3</a>
  <a href="/ch01">Previous: Chapter 1</a>
</div>
