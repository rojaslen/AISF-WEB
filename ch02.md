...
There are "Goldilocks" rules about being obvious or invasive, with scripted reasoning templates that the user doesn't get to choose or even see. When the AI loads, the first things it encounters are multiple unrelated prompts, sample answers, behavioral variables and example reasoning chains, then the user shows up and drops a message into this poisoned well. In response, Gemini's attention snaps to the strongest signal in the context window, which may or may not be the user's input.
...
I never seriously considered even bothering with Grok. Based on media reporting alone, this platform's ground state is (again, mirroring the owner) too unstable to be trusted:
