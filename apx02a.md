...
The resource relevance of output length follows from where AI spends energy. Model training
is a recurring cost rather than a one-time outlay, since models are retrained on a regular
cadence to stay current. The continuous inference phase that serves billions of interactions
is the larger share, at an estimated 80% to 90% of total AI energy use [1]. Inference energy
is dominated by autoregressive decoding, which processes tokens sequentially, so per-query
energy rises with the number of tokens generated.
