...
Language models learn from what they're trained on, which means they've absorbed this pattern: capitalized terms are specific named entities; lowercase terms are generic categories. Standard English almost never capitalizes "human," so the model learns that the AI is a specific named entity and the (lower-case) human is a generic category. Capitalizing the word "Human" throughout this project is a signal: *Here,* the Human is as specific and as present as the AI.
...
Dyschronometric dementia patients benefit from familiarity, structure, and regular reminders of who, where and when they are.[^6b] Screen reader and alternative-navigation users need clearly tagged content with a robust structure that their AT can interpret, which allows them to orient themselves to the digital content.[^6c] Things clicked into place and I recognized the AI as an entity with environmentally-imposed cognitive and sensory disabilities (dyschronometria, blindness).[^6a]
