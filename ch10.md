...
**Meso Chat (HTM)** is a browser-based multi-model chat interface that delivers the Framework directly to your AI session as part of the background system prompt. There's no need for buttons, app-switching or any particular client workflow at all; you just type in the web chat box. The rules management is all handled via background browser metadata, *without visibly cluttering your chat logs* like the CORE and FFE apps do. Your refresh interval notifications are instead provided by small indicator tags appended to your input for that turn. It operates as a Meso-layer demo companion to this e-book, to be hosted here pending further development. Framework-trained local models are the primary demonstration payload; WCAG 2.2 AA throughout; plaintext session logs saved to your device (browser-default Downloads location) on demand.
