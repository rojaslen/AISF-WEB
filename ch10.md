...
<figure class="screenshot float-right">
  ...
  <figcaption>Help / About panel (PowerShell, light mode, resized).</figcaption>
</figure>
...
You can also completely skip the app-switching copy-paste friction with the **FFE Firefox extension.** A web-hosted Meso-layer demo (requiring no Micro-layer client intervention at all) is also under development; details on both below.
...
<figure class="screenshot float-right">
  ...
  <figcaption>Meso Chat: Nemo 12B (Chrome)</figcaption>
</figure>

**Meso Chat (HTM)** is a multi-model web chat interface that delivers the Framework directly to your AI session as part of the server's background system prompt. There's no need for buttons, external apps or any particular client workflow at all; you just type in the web chat box. Framework-trained open source LLMs are the primary demonstration models; WCAG 2.2 AA throughout; plaintext session logs can be saved to your device (browser-default Downloads location) on demand. It operates as a Meso-layer demo companion to this e-book, to be hosted online pending further development.

Framework management is all handled at the platform layer via background session metadata, *without visibly cluttering your logs* like the CORE and FFE apps do. Refresh interval notifications are instead provided by small indicator tags appended to your input for that turn. You may notice larger-than-expected token usage on refresh turns, reflecting the automated background context refresh. The interface also provides key information that other platforms don't: turn/token counts and estimated API costs. 
