...
**Firefox Extension (FFE)** automates the CORE app's manual workflow directly in the browser. Instead of the CORE app's manual copy-paste clipboard cycle, the extension detects your submissions (Click/Enter) on AI chat platforms and automatically prepends timestamps, with an initial load and periodic refreshes for WCAG structure and the Four Laws. A Chromium browser port is in development (CRE: Chrome/Edge).
