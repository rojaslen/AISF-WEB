...
The CORE app is a fully accessible client-side service pack, free for personal use (CC BY-NC-ND 4.0). It started as a lightweight (< 1MB) Windows PowerShell middleware app equipped with three core stability measures: timestamps, WCAG structure and the Four Laws. It respects your device's light/dark display settings, and includes a full Help/About panel with usage instructions and keyboard shortcuts. I routinely use the Linux version of this simple clipboard-based tool to successfully produce stable, multi-hour sessions spanning dozens of turns with little to no hallucination, drift or unwanted behavior.

*(The "light mode" screenshots on this page show the Windows-only PowerShell version, which is no longer in active development but remains available. The dark mode image is of the cross-platform Python/PyQt6 version (CORE), functional in both Windows and Linux (pictured). Its larger download size reflects the cleaner and more consistent aesthetics, added QoL features and a bundled Python runtime — the same three-function workflow and full WCAG 2.2 AA compliance still applies on both platforms.)*
...
The Framework is also being ported to other systems and architectures:
...
**Meso Chat (HTM)** is a browser-based multi-model chat interface that delivers the Framework directly in your browser. There's no Micro-layer clipboard window-swapping workflow; it operates as a Meso-layer demo companion to this e-book, to be hosted here pending further development. Framework-trained local models are the primary demonstration payload; WCAG 2.2 AA throughout; plaintext session logs saved to your device (browser-default Downloads location) on demand.
...
<figure class="screenshot float-right">
  ...
  <figcaption>Meso Chat — Mistral (Chrome browser, local)</figcaption>
</figure>

**Firefox Extension (FFE)** fully automates the CORE app's manual workflow directly in the browser. Instead of the CORE app's manual copy-paste clipboard cycle, the extension detects your submissions (Click/Enter) on AI chat platforms and automatically prepends timestamps, with an initial load and periodic refreshes for WCAG structure and the Four Laws. A Chromium browser port is in development (CRE: Chrome/Edge).

**Local Model Training (OLM)** embeds AISF principles directly into locally-hosted AI models themselves through QLoRA fine-tuning. Full methodology and results are in Chapter 8 (technical data in Appendices 2 & 3). Trained models will be hosted for download at a later date.
