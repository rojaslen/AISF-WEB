...
1. **"Does this actually line up with what people want from AI, or is it just my personal preoccupation?"**

    Fair question, and there's now a way to check. Anthropic's first Public Record survey, roughly 52,000 Americans with YouGov fieldwork in late 2025, was [published](https://www.anthropic.com/news/anthropic-public-record){: target="_blank" rel="noopener noreferrer" } on June 12, 2026.[^Fd] I didn't design AISF from it. The framework was being written publicly from November 2025 and placed under version control that December, months before this survey appeared; a provisional patent was already on file when it did. The order is a matter of record. Even so, the priorities people ranked highest line up, almost item for item, with what AISF was already built to do.

    In plain terms: the second-highest *hope* people have for AI is helping people with disabilities (36%), and AISF enforces accessibility in the AI's output, with the Copilot Digital Accessibility tool already finished, security-reviewed, validated by a blind screen-reader (JAWS) user in testing, and later deployed to multiple users for active production work. Privacy is the single thing most people want government to act on (56%), and AISF runs entirely on your own device and sends nothing anywhere. Child safety is close behind (52%), which is exactly what the child-safe model work is about. And one of the top *fears* is being fed misinformation (52%), which is the whole reason the Four Laws exist: an AI that doesn't quietly make things up.

    The sharpest finding, for a solo developer especially: only 15% of Americans trust AI companies to make these decisions, the lowest of any group asked, while 43% trust independent experts, the highest. AISF is independent, and it's built on an outside public standard (WCAG) rather than a lab grading its own homework. None of this means the public has heard of AISF or endorsed it; it hasn't. It means the problems AISF targets are the ones a large, politically broad cross-section of people already say they care about. That part is worth noticing.

2. **"Why should I trust a solo developer with no institutional affiliation?"**
...
3. **"Why PowerShell? Why Windows-only?"**
...
5. **"If this is so simple, why hasn't anyone done it?"**
...
6. **"Why doesn't this cover hallucination detection?"**
    ...
    Without implausibly complex heuristics that would have to dynamically adapt to the user's input, detecting hallucinated context poisoning is impossible. Prevention sidesteps all of that, so I extracted the core principles of AISF from the detection pipeline and shelved the engine itself. That detection engine is the only part that couldn't adapt to the environment. Its adaptive traits -- session state evaluation, compliance drift monitoring, domain classification, behavioral intervention sequencing -- survived the change intact and propagated forward into all subsequent lineages. 

7. **"This is just prompt engineering."**
...
8. **"How is this different from system prompts?"**
...
9. **"Client-side injection sounds like a security vulnerability."**
    ...
    AISF produces absolutely nothing you can't preview or type in manually yourself, it just saves you the time and keystrokes. The browser extension branch automates the same process for a more frictionless user experience. AISF doesn't store, edit, or intercept data at any point in the chain. It has no connectivity functions, it can't store data remotely, and it's designed not to need access to platform APIs for flexibility. User sovereignty means that all of its operations are performed only on the user's device because sovereignty also means privacy, which platforms have never taken seriously.

10. **"You barely mentioned sycophancy. Can the developers just train that out?"**
...
11. **"Won't the Four Laws cause the AI to amplify pre-existing mental health issues, if they make it follow user directions more reliably?"**
...
12. **"The AI industry talks about 'compute' a lot, why do you barely even mention it?"**
...
13. **"Why is an anti-hallucination tool talking about Texas power grids?"**
...
14. **"Won't the platforms just block it?"**

    They certainly can, even if unintentionally. Early 2026 demonstrated what Meso-layer interventions actually look like in practice: usage limits adjusted without notice, billing caps introduced mid-cycle, tool access throttled, session capabilities changed between turns (see Chapter 10). WCAG structure and timestamps remain reasonably reliable across platforms under these conditions; Four Laws compliance is a different story. Claude and Copilot can be stabilized with relatively little effort. ChatGPT requires multiple stabilization attempts, by which point free-account usage is consumed and the model is hot-swapped with a new downgraded instance, making the exercise largely pointless for free-account holders. Gemini does not stabilize reliably under current (2026 April) conditions.

    Specifically blocking the Framework is a different question. A platform operator could scan for, detect and strip out its pre-formatted data from user inputs or censor it as a forbidden topic; it's easy enough to spot. But consider the optics of doing so: they would be choosing to proactively deny accessibility requests (*ADA & §508 regulatory compliance*), timestamps (user's content-neutral task history), and behavioral guidelines (stable & reliable = increased user satisfaction & engagement). Platforms already drawing coverage for quietly degrading service are unlikely to also publicly block accessibility requests.

    During testing, most platforms had the Framework stored in account preferences; Copilot was the exception (the feature newly exists, as yet unused). When a platform goes fully adversarial whether intentionally or as a side effect of other measures as indicated above, those stored preferences are effectively ignored by the platform. That leaves only Micro injection via the chat input, and there is a hard ceiling on what client-side mediation can accomplish when the platform is actively working against it.

    And simply as a practical matter, the Framework's workflow is indistinguishable from a user manually typing out content in any external editor and then copy-pasting it into the AI chatbox, because that's exactly what it is (as discussed in the "security vulnerability" item above). Ordinarily, *presence in active context is generally sufficient.* Short of blocking users from requesting output formatted or processed in a particular way — which would break normal usage — there's no clean way to filter it. However, platform-level service changes can render parts of it ineffective.

15. **"Does it break when the context window fills up?"**
